...
class VoyagerMediaController extends Controller
{
    ...
    public function new_folder(Request $request)
    {
        ...
        $new_folder = $this->directory . '/' . $request->new_folder;
        $success = false;
        $error = '';
    ...
    public function delete(Request $request)
    {
        ...
        $path = str_replace('//', '/', Str::finish($request->path. '/' . $this->directory, '/'));
        $success = true;
        $error = '';
    ...
    public function move(Request $request)
    {
        ...
        foreach ($request->get('files') as $file) {
            $old_path = $this->directory .'/'. $path . $file['name'];
            $new_path = $this->directory .'/'. $dest . $file['name'];
    ...
    public function upload(Request $request)
    {
        ...
        try {
            $realPath = Storage::disk($this->filesystem)->getDriver()->getAdapter()->getPathPrefix() . "/" . $this->directory;
            ...
            $file = $request->file->storeAs($request->upload_path . '/' . $this->directory, $name . '.' . $extension, $this->filesystem);
                ...
                if ($request->file->getClientOriginalExtension() == 'gif') {
                    ...
                    if (property_exists($details, 'thumbnails') && is_array($details->thumbnails)) {
                        foreach ($details->thumbnails as $thumbnail_data) {
                            ...
                            $thumbnail_file = $request->upload_path . '/' . $this->directory . $name . '-' . ($thumbnail_data->name ?? 'thumbnail') . '.' . $extension;
    ...
    public function crop(Request $request)
    {
        ...
        try {
            if ($createMode) {
                ...
                $destImagePath = preg_replace('#/+#', '/', $request->upload_path . '/' . $this->directory . '/' . $newImageName);
            } else {
