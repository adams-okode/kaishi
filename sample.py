...
pgConn2 = psycopg2.connect("dbname=ibDB user=autotrader password=autotrader")

d = current_time - datetime.timedelta(hours=100)
dt1 = pd.read_sql(
    f"SELECT tclose FROM es_5mins WHERE timehist BETWEEN {d} AND {current_time} ORDER BY timehist DESC LIMIT 250", pgConn2)

dt = [1, 2, 3, 4, 5, 6, 7, 8, 9, 2, 7, 8, 9, 6, 5, 4, 2, 3, 15]

kaufmanAdaptiveMovingAverage(5, 2, 10, dt1.values)
